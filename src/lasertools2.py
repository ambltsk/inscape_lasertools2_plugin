...
Modified by Alexandr Balatsky 2024

based on original lasertools, https://github.com/ChrisWag91/Inkscape-Lasertools-Plugin
...
class laser_gcode(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        add_argument("--laser-command-engrave", dest="laser_command_engrave", default="S100", help="Laser gcode command Engrave")
        ...
        add_argument("--laser-param-speed", type=int, dest="laser_param_speed", default="150", help="Laser speed for Parameter (mm/min)")
        add_argument("--laser-engrave-speed", type=int, dest="laser_engrave_speed", default="1500", help="Laser speed for Parameter (mm/min)")
        ...
        add_argument("--out-cut-color", dest="out_cut_color", default="",	help="Defines which tab is active")
        add_argument("--engrave-color", dest="engrave_color", default="",	help="Defines which tab is active")
        add_argument("--fill-color", dest="fill_color", default="",	help="Defines which tab is active")
    ...
    def engraving(self):
        ...
        if gcode_pass != '':
            if self.cut_passes > 1:
                for _ in range(1, self.cut_passes + 1):
                    if self.options.z_stepdown == 0:
                        gcode += "\nG90 \n" + gcode_pass
                    else:
                        gcode += "\nG91 \nG0 Z%s \nG90 \n" % self.options.z_stepdown + gcode_pass
...
################################################################################
    def effect(self):
        ...
        if self.options.add_infill and len(self.infill_paths) > 0:
            self.svg.selected_paths = self.infill_paths
            self.area_fill()
    ...
    def split_by_color(self):
        """
        Splits the found contours in the document by a
        certain stroke color. The color of the split is
        determined from the options.
        """
        self.infill_paths = {}      # Closed paths that will be used to fill the area
        self.engrave_paths = {}     # The paths that will be used to work with the engraving parameters
        self.outter_paths = {}      # The paths that will be used to work with the cutting parameters last (external contours)
        self.other_paths = {}       # Other offices that will be processed after the engraving, but before the external contours

        def _color(path):
            """
            Gets the outline color from the outline style.
            """
            try:
                style = dict([style.split(':') for style in path.get('style','').split(';')])
                return style.get('stroke','#000000')
            except:
                return '#000000'

        for layer in self.paths:
            for path in self.paths[layer]:
                if _color(path) == self.options.out_cut_color:
                    self.outter_paths[layer] = self.outter_paths[layer] + [path] if layer in self.outter_paths else [path]
                elif _color(path) == self.options.engrave_color:
                    self.engrave_paths[layer] = self.engrave_paths[layer] + [path] if layer in self.engrave_paths else [path]
                elif _color(path) == self.options.fill_color:
                    self.infill_paths[layer] = self.infill_paths[layer] + [path] if layer in self.infill_paths else [path]
                    self.engrave_paths[layer] = self.engrave_paths[layer] + [path] if layer in self.engrave_paths else [path]
                else:
                    self.other_paths[layer] = self.other_paths[layer] + [path] if layer in self.other_paths else [path]


    def sort_paths(self, layers):
        """
        Sorts contours in the dictionary by the "order"
        attribute, if available.
        """
        def sort_func(value):
            return value.get('order', 'z')

        for layer in layers:
            layers[layer].sort(key = sort_func)
